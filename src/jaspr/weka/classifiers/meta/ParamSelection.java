...
public class ParamSelection
    ...
    protected void findParamsByCrossValidation(int depth, Instances trainData,
        ...
        if (depth < m_CVParams.size()) {
            ...
            if (m_EvaluationMetric == EvaluationMetric.AUC ||
                ...
                if ((m_BestPerformance == Integer.MAX_VALUE) || (score > m_BestPerformance)) {
                    m_BestPerformance = score;
                    m_BestClassifierOptions = createOptions();
                }
            } else if (m_EvaluationMetric == EvaluationMetric.ER) {
                if ((m_BestPerformance == Integer.MIN_VALUE) || (score < m_BestPerformance)) {
                    m_BestPerformance = score;
                    m_BestClassifierOptions = createOptions();
                }
            }
    ...
    public void setOptions(String[] options) throws Exception {
        ...
        String evalMetricString = Utils.getOption('E', options);
        if (evalMetricString.length() != 0) {
            setEvaluationMetric(EvaluationMetric.valueOf(evalMetricString));
        } else {
            setEvaluationMetric(EvaluationMetric.SR);
        }
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        if (m_EvaluationMetric == EvaluationMetric.AUC ||
                m_EvaluationMetric == EvaluationMetric.SR) {
            m_BestPerformance = Integer.MAX_VALUE;
        } else if (m_EvaluationMetric == EvaluationMetric.ER) {
            m_BestPerformance = Integer.MIN_VALUE;
        }
    ...
    public EvaluationMetric getEvaluationMetric() {
        return m_EvaluationMetric;
    }

    public void setEvaluationMetric(String metric) {
        setEvaluationMetric(EvaluationMetric.valueOf(metric));
    }

    public void setEvaluationMetric(EvaluationMetric metric) {
